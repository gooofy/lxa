...
#undef SAVEDS
	 ...
	 */
	#define INLINE          static __inline
	#define STDARGS         /* no equivalent needed for cdecl-ish gcc */
	#define ASMCALL         /* function-attribute, not needed; per-arg __asm() handles regs */
	#define ASM             ASMCALL
	#define REGCALL         /* compiler-chosen registers; not really needed */
	 ...
	 * (-mregparm style) plus explicit __asm("dN") on critical declarations.
	 *
	 * For the BGUI dispatcher entry points this is fine because METHOD()
	 * is redefined below to use proper per-argument __asm() clauses.
	 */
	#define REG(reg)
#endif
...
#else
/* lxa: upstream had `typedef IPTR ULONG` (reversed + missing semicolon).
 * IPTR is an integer-sized pointer-holder.  On classic m68k it's 32-bit
 * unsigned, identical to ULONG.  Pull in exec/types.h so ULONG is known
 * regardless of inclusion order from the BGUI master header. */
#include <exec/types.h>
typedef ULONG IPTR;
#define BNULL 0
/* class-protos.h was auto-generated by MakeProto for the AROS port and
 * contains AROS's RAWARG type in varargs prototypes.  On classic m68k
...
#else
#if defined(__GNUC__) && defined(__mc68000__) && !defined(__AROS__)
  /* lxa: m68k-amigaos-gcc (bebbo). Per-argument __asm() register binding. */
  #define METHOD(f,mtype,m) SAVEDS IPTR f(Class *cl __asm("a0"), Object *obj __asm("a2"), Msg _msg __asm("a1"), APTR _global __asm("a4")) \
	{ mtype __attribute__((unused)) m = (mtype)_msg;
  #define METHODPROTO(f,mtype,m) SAVEDS IPTR f(Class *cl __asm("a0"), Object *obj __asm("a2"), Msg m __asm("a1"), APTR _global __asm("a4"))
  #define METHOD_END }
