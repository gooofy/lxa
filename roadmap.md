...
| 136-b2 | bgui.library source build: all 49 source files compile. `bgui_compilerspecific.h` extended with `getreg(REG_A4)`/`putreg()`/`geta4()` (a4 read via `register IPTR _v __asm("a4")`, used by listclass.c and dgm.c for the BGUI library global pointer that lives in A4 under `-fbaserel`), and with portable `AROS_SLOWSTACKTAGS_PRE[_AS]/ARG/POST` and `AROS_SLOWSTACKMETHODS_PRE[_AS]/ARG/POST` macros (varargs-as-TagItem-array trick — same convention as NDK `<inline/...>` stubs — used by classes.c/dgm.c/miscc.c). `classes.c` guards SAS/C `<dos.h>` for bebbo. `request.c` adds `#include <bgui/bgui_macros.h>`. `build_include/bgui/bgui_macros.h` symlink retargeted to `../../include/bgui_macros.h` (the prior libraries_bgui_macros.h target was circular). Compile sweep: **OK 49 / Fail 0**. CMake target and link step next. | v0.9.14 |
 | 136-b4 | bgui.library calling-convention fixes — Typeface RENDERS character grid: this phase resolved the cascading ABI mismatches that prevented the lxa-built BGUI from running. Three root causes fixed: (R5) **Asm*Pool register-arg ABI mismatch** — BGUI's task.c declared `AsmCreatePool/AsmDeletePool/AsmAllocPooled/AsmFreePooled` with register-argument signatures (D0/D1/A0 etc.) but `lxa_shims.c` defined them with plain C calling convention; bebbo silently linked them, so arg registers were never read. Added `__asm("xN")` register bindings to all four shims; removed a duplicate `AsmFreePooled` definition. (R6) **libgcc baserel pollution** — bebbo's gcc spec contains `*link: %{shared:-shared -m amiga_bss -amiga-datadata-reloc -fl libb}`, meaning `-shared` silently swaps the standard libgcc/libnix for the **baserel** variants in `libb/`. The result: `___udivsi3` was emitted as `movea.l -23972(a4),a0; jmp -156(a0)` reading the dispatch table via an A4 base register that BGUI never initialises (lxa was built without `-fbaserel` and uses `getreg(REG_A4)` only as an opaque cookie). Symptom: Typeface's first division during window-size calculation jumped to garbage and trapped. (R7 — the breakthrough) **libnix baserel pollution** — same `-fl libb` mechanism also pulled baserel `_memcpy`, `_sprintf`, `_memset` from `libb/libnix.a`, which read SysBase via `movea.l -23988(a4),a6`. **Final fix in `src/bgui/CMakeLists.txt`**: replaced `-shared` with `-Wl,-shared,-m,amiga_bss,-amiga-datadata-reloc` (passes the linker flags directly, skipping the `-fl libb` library-search-path swap). Verified post-fix: `_memcpy` now uses `movea.l 0x224a, a6` reading the absolute `_SysBase` symbol that BGUI's LibInit at `lib.c:364` sets via `SysBase = syslib;`. Defense-in-depth: `kprintf` decimal printer in `lxa_shims.c` rewritten to use repeated subtraction (avoiding any `/` or `%` that could pull in `___udivsi3`). Mechanical port-side cleanup: ~1050 `REG(xN, type name)` macro sites across 48 BGUI .c/.h files mass-rewritten to bebbo's postfix `type name __asm("xN")` form via `/tmp/fix_reg.py` (REG was empty under bebbo's REGPARAM macro). Test result: `typeface_gtest --gtest_filter="*StartupOpensWindow*"` **PASSES** (591 ms); `lxa_capture_window()` shows the Typeface character grid rendering correctly. Missing vs FS-UAE reference (`others/fs-uae-real-2604221933-01.png`): window border/title bar and vertical prop scrollbar; window size 194×138 vs reference 280×195 — investigation deferred to next phase. Full suite: 63/68, same 5 pre-existing baseline failures, zero regressions. All temporary diagnostic LPRINTFs (PROBE: OpenLibrary post-InitResident in `exec.c`, PROBE: NONROM op_illg in `lxa_dispatch.c`, PROBE: LoadSeg hunk alloc in `lxa_dos.c`) reverted; all temporary `kprintf` traces in `src/bgui/lib.c`, `src/bgui/task.c`, `src/bgui/windowclass.c` removed or wrapped via the `WW()` macro (currently `#define WW(x)`, disabled). | v0.9.16 |
