...
- [x] Closed the remaining public DOS stub surface, including startup/init, record-locking, CLI bootstrap, task-context helpers, DOS-list mutation, media-control calls, segment registration, parsing helpers, and owner metadata.
- [x] Kept each closure anchored against RKRM/NDK plus AROS review, while documenting the few intentional lxa-specific safety tightenings and hosted-environment divergences.
- [x] Extended direct coverage across `Tests/Dos/*`, `dos_gtest`, console regressions, and the `Tests/Exec/Library` entry-point probes so the finished DOS surface stays pinned down without a per-function checklist in the live roadmap.
...
- [x] Closed the remaining public Utility tag-item helpers: `PackBoolTags()`, `FilterTagChanges()`, and `ApplyTagChanges()`.
- [x] Verified each helper against the AROS implementation and kept the aligned semantics covered in `tests/utility/tagitems` plus the `Tests/Exec/Library` entry-point probe.

### Phase 85-95 complete (`0.8.37`-`0.8.46`)

- [x] Phase 85 (`graphics.library`): closed the remaining public graphics stub surface, including GEL/sprite animation, user-copper helpers, superbitmap sync/copy, chipset negotiation, and sprite-data management, with direct graphics/layers regression coverage.
- [x] Phase 86 (`intuition.library`): closed the remaining Intuition stub surface around legacy `OpenIntuition()` and synthetic-input `Intuition()` compatibility, keeping hosted input delivery aligned with active-window IDCMP behavior.
- [x] Phase 87 (`icon.library`): closed the planar icon-management surface, including default icons, control/query tags, drawing/layout helpers, rectangle queries, and selected-color transforms.
- [x] Phase 88 (`diskfont.library`): added the V47 outline-engine wrappers, `.otag` open/close flow, diskfont write helpers, and the hosted charset lookup surface.
- [x] Phase 89 (`workbench.library`): closed the hosted Workbench object/app-window/drop-zone/control/query APIs needed by the current compatibility target set.
- [x] Phases 90-93 (`timer.device`, `clipboard.device`, `gameport.device`, `trackdisk.device`): finished the remaining device expunge/pending-I/O/media-control surfaces and kept them covered under the device suites plus direct Exec probes.
- [x] Phase 94 (`mathffp.library`): finished the hosted init path so the library no longer logs an unimplemented startup stub.
- [x] Phase 95: completed the ROM-side stub-eradication sweep and verified that only intentional private/reserved entry points remain unimplemented, all with explicit `error: private function called` diagnostics.
