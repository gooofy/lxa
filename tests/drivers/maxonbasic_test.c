/*
 * maxonbasic_test.c - Host-side test driver for MaxonBASIC
 *
...
static char *find_apps_path(void)
{
    ...
    const char *locations[] = {
        ...
        "../../lxa-apps/MaxonBASIC/bin/MaxonBASIC",
        "../../../lxa-apps/MaxonBASIC/bin/MaxonBASIC",
        /* Absolute path */
        "/home/guenter/projects/amiga/lxa/src/lxa-apps/MaxonBASIC/bin/MaxonBASIC",
        NULL
    };
...
int main(int argc, char **argv)
{
    ...
    if (!apps_path) {
        fprintf(stderr, "ERROR: Could not find MaxonBASIC directory\n");
        fprintf(stderr, "Expected at: lxa-apps/MaxonBASIC/bin/MaxonBASIC/\n");
        return 1;
    }
    ...
    /* Load MaxonBASIC */
    printf("Loading MaxonBASIC...\n");
    if (lxa_load_program("SYS:MaxonBASIC", "") != 0) {
        fprintf(stderr, "ERROR: Failed to load MaxonBASIC\n");
        lxa_shutdown();
        return 1;
    }
    
    /* ========== Test 1: Wait for window to open ========== */
    printf("\nTest 1: Waiting for MaxonBASIC window to open...\n");
    {
    ...
    /* ========== Test 3: Let MaxonBASIC initialize fully ========== */
    printf("\nTest 3: Letting MaxonBASIC initialize...\n");
    {
        /* Run more cycles to let MaxonBASIC fully initialize */
    ...
    printf("\nTest 5: Testing mouse input...\n");
    {
        ...
        if (lxa_get_window_info(0, &win_info)) {
            ...
            check(lxa_is_running(), "MaxonBASIC still running after mouse click");
        } else {
    ...
    printf("\nTest 6: Testing quit...\n");
    {
        ...
        if (!lxa_is_running()) {
            check(1, "MaxonBASIC exited via Amiga-Q");
        } else {
            ...
            if (!lxa_is_running()) {
                check(1, "MaxonBASIC exited via close gadget");
            } else {
                printf("  Note: MaxonBASIC still running (normal - may need confirmation)\n");
