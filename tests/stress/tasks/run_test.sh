...
# Run test with a timeout (task tests may take longer)
LXA_PREFIX="." "$LXA_BIN" -c "$CONFIG_FILE" "$TEST_BINARY" > "$ACTUAL_OUTPUT" 2>&1 || true

# Check for PASS in output
if grep -q "PASS: All task stress tests passed" "$ACTUAL_OUTPUT"; then
    echo "PASS"
    exit 0
else
    echo "FAIL: Task stress test did not pass"
    echo "Output:"
    cat "$ACTUAL_OUTPUT"
    exit 1
fi
